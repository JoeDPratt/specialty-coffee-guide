...
export function transformProduct(raw: any): Product {
    ...
    return {
        roaster: {
            ...
            logo_img_url:
                raw.coffee_roasters.roaster_images.find((img: any) => img.is_primary)
                    ?.image_url || "",
            alt_text:
                raw.coffee_roasters.roaster_images.find((img: any) => img.is_primary)
                    ?.alt_text || "",
        ...
        flavours: raw.coffee_flavours_identified.map(
            (f: any) => f.canonical_flavour,
        ),
        roasts: raw.coffee_roasts.map(
            (r: any) => r.default_roasts?.display_name ?? "",
        ),
        ...
        provenance: {
            origin_countries: (provenance.coffee_origin_countries || []).map(
                (c: any) => c.default_origin_countries.display_name,
            ),
            origin_regions: (provenance.coffee_origin_regions || []).map(
                (r: any) => r.default_origin_regions.display_name,
            ),
            producers: (provenance.coffee_producers || []).map(
                (p: any) => p.default_coffee_producers.display_name,
            ),
            altitude: {
                min: provenance.altitude_min ?? null,
                max: provenance.altitude_max ?? null,
            },
            varietals: (provenance.coffee_varietals || []).map(
                (v: any) => v.default_varietals?.display_name ?? "",
            ),
            processes: (provenance.coffee_processes || []).map(
                (p: any) => p.default_coffee_processes.display_name,
            ),
        },
